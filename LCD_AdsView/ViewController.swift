...
class ViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        self.perform(#selector(ViewController.upArrayClick), with: nil, afterDelay: 4.3)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case 2:
            ...
            let adsView = LCDAdsView.show(view: cell!.contentView, frame: CGRect(x:0, y:0, width:UIScreen.main.bounds.size.width, height:cell_H), adsType: .Half_H, imageUrls: images1,itemSize:CGSize(width:UIScreen.main.bounds.size.width/2 - 2, height:cell_H))
            adsView._LCDAdsViewClosures = { (itemIdex, isSelect) in
                if isSelect {
                    let aler = UIAlertController(title: "点击了第\(itemIdex)张图片", message: nil, preferredStyle: .alert)
                    let one = UIAlertAction.init(title: "确定", style: .default, handler: { (UIAlertAction) in
